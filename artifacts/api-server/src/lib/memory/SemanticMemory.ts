 ...
 *
 * Gracefully degrades when DATABASE_URL is not set — all methods return
 * empty/fallback values without throwing.
 */
...
export class SemanticMemory {
  ...
  private _hasDb(): boolean {
    return !!process.env.DATABASE_URL;
  }

  /**
   * Get top-performing patterns from the performance matrix.
   */
  async getTopPatterns(symbol: string): Promise<MemoryEntry[]> {
    if (!this._hasDb()) return [];
    try {
